...
def main():
    model = CustomResnet3DBuilder.build_resnet_50((64, 64, 64, 1), 3)
    model.summary()

    random_input = tensorflow.random.uniform((1, 64, 64, 64, 1))
    output = model.predict(random_input)
    print(output)


def build_model(shape: tuple):
    model = CustomResnet3DBuilder.build_resnet_50(shape, 3)
